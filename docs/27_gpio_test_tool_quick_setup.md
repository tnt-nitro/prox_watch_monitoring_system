# GPIO Test Tool - Schnell-Setup (Dateien manuell erstellen)
...
Dann die Dateien manuell erstellen. Siehe die vollständigen Dateiinhalte in den nächsten Abschnitten.

## Dateien erstellen

### 1. cmd/ledtest/main.go
...
### 4. internal/gpio/beeper.go

```bash
cat > internal/gpio/beeper.go << 'EOF'
package gpio
...
### 5. internal/gpio/button.go

```bash
cat > internal/gpio/button.go << 'EOF'
package gpio
...
### 6. internal/gpio/init.go

```bash
cat > internal/gpio/init.go << 'EOF'
package gpio
...
### 7. internal/gpio/init_mock.go

```bash
cat > internal/gpio/init_mock.go << 'EOF'
//go:build !raspberry
// +build !raspberry
...
### 8. internal/gpio/pin_raspberry.go

```bash
cat > internal/gpio/pin_raspberry.go << 'EOF'
//go:build raspberry
// +build raspberry
...
### 9. internal/gpio/pin_mock.go

```bash
cat > internal/gpio/pin_mock.go << 'EOF'
//go:build !raspberry
// +build !raspberry
...
### 10. internal/neopixel/strip.go

```bash
cat > internal/neopixel/strip.go << 'EOF'
package neopixel
